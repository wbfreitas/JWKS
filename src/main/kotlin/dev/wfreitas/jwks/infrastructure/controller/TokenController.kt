...
import dev.wfreitas.jwks.infrastructure.controller.dto.TokenValidationRequest
...
class TokenController(
    ...
    fun validate(@RequestBody request: TokenValidationRequest): ResponseEntity<Void> {
        val valid = validateTokenUseCase.execute(request.token)
        return if (valid) ResponseEntity.ok().build()
